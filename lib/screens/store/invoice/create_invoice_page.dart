...
class _CreateInvoicePageState extends State<CreateInvoicePage> {
  ...
  double get _subTotal =>
      _cart.values.fold(0, (s, c) => s + c.lineTotal);
  ...
  double get _totalPaid    => (_useCash ? _cashAmount : 0) + (_useOnline ? _onlineAmount : 0);
  double get _amountDue    => _total - _totalPaid;

  // Build payments list for API
  List<Map<String, dynamic>> get _paymentsPayload {
    final list = <Map<String, dynamic>>[];
    if (_useCash   && _cashAmount   > 0) list.add({'method': 'cash',   'amount': _cashAmount});
    if (_useOnline && _onlineAmount > 0) list.add({'method': 'online', 'amount': _onlineAmount});
    return list;
  }

  String _fmt(double v) => '₹${v.toStringAsFixed(0).replaceAllMapped(
      RegExp(r'(\d)(?=(\d{2})+(\d)\b)'), (m) => '${m[1]},')}';
            ...
            _Card(
              ...
              child: _cart.isEmpty
                ...
                child: Center(
                  ...
                  : Column(
                children: [
                  ..._cart.entries.map((entry) => _CartRow(
                    item: entry.value,
                    onIncrement: () =>
            ...
            children: [
              ...
              if (_discountType != null) ...[
                ...
                TextField(
                  ...
                  decoration: _dec(_discountType == 'flat'
                      ? 'Amount in ₹'
                      : 'Percent 0–100'),
                ),
              ],
            ...
            children: [
              ...
              if (!isPending) ...[
                ...
                Builder(builder: (_) {
                  ...
                  final due =
                  (_total - paid).clamp(0, double.infinity);
                  return Column(children: [
                    _SummaryRow(
                        label: 'Paid now', value: _fmt(paid)),
                    _SummaryRow(
                      label: 'Remaining due',
            ...
            children: [
              ...
              if (cartEntries.isNotEmpty)
                Container(
                  ...
                  child: Text('${cartEntries.length} added',
                      style: const TextStyle(
                          fontSize: 10,
                          fontWeight: FontWeight.w600,
                          color: AppColors.green)),
            ...
            children: [
              Expanded(
                child: Row(
                  children: [
                    Flexible(
                      child: Text(item.product.prodName,
                          style: const TextStyle(
                              fontSize: 13,
                              fontWeight: FontWeight.w600,
                              color: AppColors.textDark),
          ...
          GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(6),
                border: Border.all(
                    color: isEnabled ? color : AppColors.border, width: 1.5),
              ),
              child: isEnabled
...
class _DiscBtn extends StatelessWidget {
  ...
  final bool active;
  final VoidCallback onTap;
  const _DiscBtn({required this.label, required this.active, required this.onTap});
